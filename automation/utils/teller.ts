...
export const getAccountBalance = async (
  ...
  try {
    const response = await axios.get(
      ...
      { httpsAgent: getAgent(), auth: { username: token, password: '' } },
    );
    return response.data;
  } catch (err) {
    console.error(
      `Could not get account balance for account id ${accountId}`,
      err,
    );
...
export const getTransactions = async (
  ...
  try {
    ...
    console.error(
      `Could not get transactions for account id ${accountId}`,
      err,
    );
...
export const TOKEN_TO_ACCOUNTS: Record<string, string[]> = Object.fromEntries(
  Object.entries({
    [CHASE_TOKEN]: [
      CHASE_CHECKING,
      CHASE_FREEDOM_FLEX,
